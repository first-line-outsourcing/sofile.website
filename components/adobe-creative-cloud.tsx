...
function TrustCard() {
  return (
    <article className="sofile-gradient-border-card flex h-full flex-col p-6 md:p-7">
      ...
      <div className="mt-5 space-y-2.5" role="list" aria-label="Adobe partnership badges">
        {adobeBadges.map((badge) => (
          <div
            key={badge.alt}
            role="listitem"
            className="rounded-lg bg-black/25 px-3.5 py-2.5"
          >
            <Image
              src={badge.src}
              alt={badge.alt}
...
function SupportedAppsCard() {
  return (
    <article className="sofile-gradient-border-card sofile-gradient-border-card--no-right relative flex h-full flex-col overflow-visible">
      {/* Background — Adobe app icons, right side */}
      <div
        className="pointer-events-none absolute -right-16 -top-16 bottom-0 z-[2] w-[66%] max-w-[460px]"
        aria-hidden
      >
        <Image
          src="/images/adobe-apps.png"
          alt=""
          fill
          sizes="(max-width: 1024px) 55vw, 440px"
          className="object-contain object-right object-bottom scale-[1.08] drop-shadow-[0_0_34px_rgba(59,130,246,0.16)]"
        />
      </div>

      <div className="relative z-[3] flex flex-col p-6 md:p-7 md:pr-[46%] lg:pr-[50%]">
        <h3 className="text-base font-semibold text-foreground md:text-lg">
        ...
        <ul className="mt-5 flex max-w-sm flex-col gap-y-3">
          {supportedApps.map((app) => (
            <li key={app} className="flex items-center gap-2.5 text-sm text-foreground/90">
              <span className="sofile-glass-check-circle">
        ...
        <p className="mt-5 max-w-sm pt-4 text-[11px] leading-relaxed text-muted-foreground sm:max-w-none">
